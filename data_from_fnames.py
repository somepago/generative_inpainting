...
class DataFromFNames(Dataset):
    ...
    def __init__(self, fnamelists, shapes, random=False, random_crop=False,
                 ...
                 return_fnames=False, filetype='image',adv_train=False):
    ...
    def next_batch(self):
        ...
        if self.adv_train and self.has_init:
#             import ipdb ; ipdb.set_trace()
            batch_data = np.concatenate(batch_data,axis=0)
            batch_data = self.sess.run(self.adv_op, feed_dict={self.images:batch_data})[0]
            batch_data = [[x] for x in batch_data]
#             fp = '/cmlscratch/gowthami/generative_inpainting/adv.npy'
#             with open(fp,'wb') as fh:
#                 np.save(fh,batch_data)
